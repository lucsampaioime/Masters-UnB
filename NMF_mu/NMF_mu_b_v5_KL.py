...
def nmf_with_update(file_path, n_topics, n_labeled, max_iter, tol):
    ...
    for n in range(max_iter):
        # Atualiza as matrizes W e H utilizando as regras de atualização para a divergência KL
        W *= (V / (W @ H + np.finfo(float).eps)) @ H.T / \
            np.ones([V.shape[0], 1]) @ H.T
        H *= W.T @ (V / (W @ H + np.finfo(float).eps)) / \
            W.T @ np.ones([V.shape[0], 1])

        # Aplique a função 'Supress'
        W[labeled, 0] = np.max(W)
        W[labeled, 1:] = 0.001

        # Calcula a divergência KL entre V e WH
        WH = W @ H
        divergence = np.sum(
            V * np.log((V + np.finfo(float).eps) / (WH + np.finfo(float).eps)) - V + WH)

        # Se o erro for menor que a tolerância, para o processo
        if divergence < tol:
            break

        print(f"Iteração {n+1}: erro = {divergence}")
...
for x in range(10):
    ...
    nmf_with_update(
        'C:/Users/lucsa/Dropbox/Data Science/Mestrado UNB/Dissertação/Experimentos/Testes/NMF/Datasets/CSTR.csv', 10, 5, 30, 1e-4)
