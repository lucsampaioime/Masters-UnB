...
for x in range(10):
    ...
    nmf_with_update(data_folder / "CSTR.csv", 5, 10, 30, 1e-4)
...
for x in range(10):
    ...
    # nmf_with_update(file_path, n_topics, n_labeled, max_iter, tol)
    nmf_with_update(data_folder / "CSTR.csv", 10, 10, 30, 1e-4)
...
for x in range(10):
    ...
    # nmf_with_update(file_path, n_topics, n_labeled, max_iter, tol)
    nmf_with_update(data_folder / "CSTR.csv", 20, 10, 30, 1e-4)
...
for x in range(10):
    ...
    # nmf_with_update(file_path, n_topics, n_labeled, max_iter, tol)
    nmf_with_update(data_folder / "CSTR.csv", 50, 10, 30, 1e-4)
...
# No cenário de classificação binária, a matriz W é usada para fazer predições binárias em vez de multiclasse.
# No nosso caso, o  que fazemos é determinar qual tópico tem o valor mais alto para cada documento e então verificar
# se esse tópico é o tópico correspondente à classe positiva.

# Aqui, np.argmax(W, axis=1) retorna o índice do tópico que tem o valor mais alto para cada documento.
#  Em seguida, nós comparamos esse índice com positive_class_index, que é o índice do tópico correspondente à classe positiva.
# O resultado dessa comparação é um array de valores booleanos (True/False), que então convertemos em 1s e 0s para corresponder
# ao formato do nosso array de classes.

# Assim, a comparação entre as previsões e as classes reais é feita. Em outras palavras,
# se o tópico com o valor mais alto para um documento específico é o tópico correspondente à classe positiva,
# consideramos essa previsão como "1" (classe positiva). Caso contrário, consideramos a previsão como "0" (classe negativa).
# Essas previsões podem então ser comparadas diretamente com as classes reais para calcular a acurácia e outras métricas de avaliação.
