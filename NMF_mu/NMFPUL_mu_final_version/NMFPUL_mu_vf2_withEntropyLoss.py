...
from sklearn.metrics import accuracy_score, precision_score, recall_score, f1_score, confusion_matrix, mean_squared_error, log_loss
...
def nmf_with_update(V, n_topics, n_labeled, max_iter, tol):
    ...
    for n in range(max_iter):
        ...
        # Calcula o Log Loss
        # Para calcular o log loss, você precisará das probabilidades da classe positiva.
        # Neste caso, você pode usar o valor de W correspondente à classe positiva como uma aproximação das probabilidades.
        probs = W[:, positive_class_index]

        # Calcula o log loss para os dados não rotulados.
        loss = log_loss(classes[unlabeled_mask], probs[unlabeled_mask])

        # Armazena o valor de log loss
...
log_loss_mean, log_loss_std = np.mean(log_loss_values), np.std(log_loss_values)
...
print(
    f"Média e desvio padrão do Log Loss: {log_loss_mean:.4f}, {log_loss_std:.4f}")
