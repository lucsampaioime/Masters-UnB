...
def kl_reconstruct_error(x, w, h):
    print("Shape of W inside training:", w.shape)
    print("Shape of H inside training:", h.shape)
    a = x
    b = torch.mm(w, h)
    kl_div = torch.where(a != 0, torch.where(
        b != 0, a * torch.log(a / b) - a + b, torch.zeros_like(a)), torch.zeros_like(a))
...
def train_unsupervised(V_train_tns, V_test_tns, H_train_tns, H_test_tns, W_init_tns, num_layers, network_train_iterations, n_components, features, lr=0.0005, l_1=0, l_2=0, verbose=False, include_reg=True):
    ...
    for i in range(network_train_iterations):
        ...
        # Use KL divergence for reconstruction error
        loss = kl_reconstruct_error(V_train_tns, dnmf_w.T, out.T)
