...
# Define number of positive examples that will be labeled
num_labeled = 1000
...
for _ in range(N):
    ...
    # Choose a subset of positive examples to be labeled
    labeled_indices = np.random.choice(
        positive_indices, num_labeled, replace=False)

    # Set 'Class' to undefined for non-positive classes and for a subset of positive examples
    df['Class'] = np.where(df.index.isin(labeled_indices),
                           positive_class, 'undefined')
